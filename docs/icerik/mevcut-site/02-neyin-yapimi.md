---
baslik: Neyin Yapımı
slug: neyin-yapimi
kaynak_url: https://mansurney.vercel.app/icerik.php?id=11&s=neyin-yapimi
menu: NEY ▾ > NEY YAPIMI
tarih: 2018-04-23
goruntulenme: 114842
kapak_gorseli: resimler/icerikler/11.jpg
ozet: "Neyin ana malzemesi olan kamışlar Hatay ilimizin Asi ırmağı kenarından kesilir."
alindi: 2026-09-14
---

# Neyin Yapımı

Neyin ana malzemesi olan kamışlar Hatay ilimizin Asi ırmağı kenarından kesilir. Yaklaşık 6 ay süren bir kurutma işlemi uygulanır. Bu kurutma işleminde kamışlar altlarına ağır bir cisim bağlanarak sallandırılır. Bu yerin serin olmasına özen gösterilir. Kamışlar kuruma işleminden sonra uygun ölçüleri belirlenerek tesviyesine başlanır.Kamışın varsa eğriliği düzeltilir.Bu işlemde buhar kullanılması daha iyidir.Başpare takılmasından sonra neylerin sesleri akort makinesi ile belirlenir.Bu belirleme sonrası neyi açma işlemi yapılır.Ney açma işinden sonra yağlanır.Bu aşamadan sonra neyzenler kendi neylerinin bakımını yapmalıdır.

---

## Editoryal notlar (yeni site için)

- Metin 2003 sürümüyle birebir aynı.
- Nokta sonrası boşluklar eksik (`başlanır.Kamışın` vb.).
- Süreç doğal olarak adım adım anlatılabilir (yeni tasarımda “süreç/timeline” bileşeni önerisi):
  1. Kamışın Asi kıyısından kesilmesi
  2. ~6 ay serin yerde, ağırlıkla sallandırılarak kurutma
  3. Ölçü belirleme ve tesviye (eğriliğin buharla düzeltilmesi)
  4. Başpare takılması
  5. Akort makinesiyle seslerin belirlenmesi
  6. Neyin açılması (perde/iç açkı)
  7. Yağlama ve teslim
- Atölye ve usta fotoğraflarıyla zenginleştirilmeli (eski sitede `kamis.JPG`, `ney111.jpg` görselleri vardı).
